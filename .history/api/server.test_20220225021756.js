...
describe('[GET] /users', () => {
	...
	it('[13] should have user1 as the first user', async () => {
		const res = await request(server).get('/users');
		expect(res.body[0]).toMatchObject({ name 'user1' });
	});
});

describe('[POST] /users', () => {
	it('[14] creates a new user in the database', async () => {
		let res = await request(server).post('/users').send({ name 'newuser' });
		expect(res.body).toMatchObject({ name 'newuser' });
	});

	it('[15] should get a 201 status', async () => {
		const res = await request(server).post('/users').send({ name 'newuser' });
