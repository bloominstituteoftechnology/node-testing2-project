...
describe('user model functions', () => {
	...
	// 	test('delete 1 user from database', async () => {
	...
	describe('[POST] /api/register', () => {
		it('[5] creates a new user in the database', async () => {
		  await request(server).post('/api/register').send({ username: 'username1', password: 'password1' })
		  const username1 = await db('users').where('username', 'sue').first()
