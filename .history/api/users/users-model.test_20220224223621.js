...
describe('[POST] /api/register', () => {
	it('[5] creates a new user in the database', async () => {
		...
		const user = await db('users').where('username', 'user').first();
		expect(user).toMatchObject({ username: 'user' });
	}, 750);
	...
	it('[8] responds with the user (user_id and username)', async () => {
		const res = await request(server)
			.post('/api/register')
			.send({ username: 'user', password: '1234' });
		expect(res.body).toMatchObject({ user_id: 2, username: 'user' });
	}, 750);
