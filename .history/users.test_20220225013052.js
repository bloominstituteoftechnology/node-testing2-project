...
describe('[POST] /users', () => {
	it('[13] creates a new user in the database', async () => {
		await request(server).post('/users').send({ user: 'user8' });
		const user8 = await db('users').where('user', 'newuser8').first();
		expect(user8).toMatchObject({ user: 'user8' });
	});

	it('[14] should get a 201 status', async () => {
		const res = await request(server).post('/users').send({ user: 'jojo' });
